...
const CMD_ENABLE: u32 = KEYFLAG_BASE;
const CMD_US:     u32 = KEYFLAG_BASE + 1;
const CMD_ABNT2:  u32 = KEYFLAG_BASE + 2;
...
impl KeyFlag {
    pub fn new() -> Self {
        unsafe {
            ...
            let mode = match reg_read_dword(REG_KEY, REG_MODE) {
                Some(1) => KfMode::Abnt2, _ => KfMode::UsIntl,
            };
            Self { enabled, mode, icon_us: HICON::default(), icon_abnt2: HICON::default(), hook: HWINEVENTHOOK::default() }
        }
    }
}
...
impl FlagModule for KeyFlag {
    ...
    fn on_init(&mut self, hwnd: HWND) {
        unsafe {
            ...
            if self.enabled { start_enforcement(hwnd, self.mode, &mut self.hook); }
        }
    }

    fn on_destroy(&mut self, hwnd: HWND) {
        unsafe { if self.enabled { stop_enforcement(hwnd, &mut self.hook); } }
    }

    fn on_timer(&mut self, hwnd: HWND, id: usize) {
        if self.enabled && id == TIMER_ENFORCE { unsafe { enforce(self.mode); } }
        let _ = hwnd;
    }

    fn on_message(&mut self, hwnd: HWND, msg: u32, _wp: WPARAM, _lp: LPARAM) -> Option<LRESULT> {
        if self.enabled && msg == WINEVENT_HOOK_MSG {
    ...
    fn append_menu(&self, hmenu: HMENU) -> bool {
        unsafe {
            ...
            let gray    = if self.enabled { MF_UNCHECKED } else { MF_GRAYED };
            let chk_us    = if self.mode == KfMode::UsIntl { MF_CHECKED } else { gray };
            let chk_abnt2 = if self.mode == KfMode::Abnt2  { MF_CHECKED } else { gray };
            let _ = AppendMenuW(hmenu, MF_STRING | chk_en,    CMD_ENABLE as usize, PCWSTR(w("KeyFlag — Enabled").as_ptr()));
            let _ = AppendMenuW(hmenu, MF_STRING | chk_us,    CMD_US as usize,     PCWSTR(w("  US International").as_ptr()));
            let _ = AppendMenuW(hmenu, MF_STRING | chk_abnt2, CMD_ABNT2 as usize,  PCWSTR(w("  ABNT2 (PT-BR)").as_ptr()));
        }
        true
    }
        ...
        match cmd {
            CMD_ENABLE => {
                ...
                unsafe {
                    ...
                    if self.enabled { start_enforcement(hwnd, self.mode, &mut self.hook); }
                    else            { stop_enforcement(hwnd, &mut self.hook); }
                }
                true
            }
            CMD_US if self.enabled => { self.set_mode(KfMode::UsIntl); true }
            CMD_ABNT2 if self.enabled => { self.set_mode(KfMode::Abnt2); true }
            _ => false,
        }
    }
}
...
unsafe fn start_enforcement(hwnd: HWND, mode: KfMode, hook: &mut HWINEVENTHOOK) {
    apply_mode(mode);
...
unsafe fn stop_enforcement(hwnd: HWND, hook: &mut HWINEVENTHOOK) {
    let _ = KillTimer(hwnd, TIMER_ENFORCE);
    if !hook.is_invalid() { let _ = UnhookWinEvent(*hook); *hook = HWINEVENTHOOK::default(); }
}
...
unsafe fn unload_others(keep: HKL) {
    ...
    for &hkl in &hkls[..n as usize] { if hkl != keep { let _ = UnloadKeyboardLayout(hkl); } }
}

unsafe fn set_preload(mode: KfMode) {
    flag_win::reg_write_str(r"Keyboard Layout\Preload", "1", mode.klid());
}
...
unsafe fn enforce(mode: KfMode) {
    ...
    if hkl != want { let _ = PostMessageW(fw, WM_INPUTLANGCHANGEREQUEST, WPARAM(0), LPARAM(want.0 as isize)); }
}
